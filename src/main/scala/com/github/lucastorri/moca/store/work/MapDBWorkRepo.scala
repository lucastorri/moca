...
//import com.github.lucastorri.moca.store.content.{ContentLink, OutLinksTransfer}
...
//  }
//
//  override def done(workId: String, transfer: OutLinksTransfer): Future[Unit] = transaction {
...
//  }
//
//  override def addWork(seeds: Set[Work]): Future[Unit] = transaction {
//    seeds.foreach(w => work.put(w.id, ws.serialize(w)))
//  }
//
//  override def links(workId: String): Future[Option[OutLinksTransfer]] = transaction {
...
//}
//
//case class ConcreteWorkTransfer(private val links: Set[ContentLink]) extends OutLinksTransfer {
//
...
//
//  def apply(other: OutLinksTransfer): ConcreteWorkTransfer = other match {
