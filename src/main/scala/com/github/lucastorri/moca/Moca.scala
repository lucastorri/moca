...
object Moca extends App with StrictLogging {
  ...
  if (config.hasRole(Master.role)) {
    Master.standBy(config.workRepo, config.scheduler, config.tasksHandler, config.bus)
  }

  if (config.hasRole(Worker.role)) {
    val start = Worker.start(config.contentRepo, config.browserProvider, config.partition, config.bus, config.dedicatedMaster) _
